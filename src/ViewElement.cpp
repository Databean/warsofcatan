...
void ViewButtonText::render() {
	renderText(ViewElement::getRect().first, ViewElement::getRect().second, texture);
}
...
void TradingView::render() {
	...
	renderRectangle(topLeft, bottomRight, std::tuple<float,float,float>(1,1,0));
...
void ConfirmationDialogue::render(){
	renderRectangle(topLeft, bottomRight, std::tuple<float,float,float>(1.,1.,1.));
